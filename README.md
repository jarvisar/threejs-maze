...
- **Something is in there with you.** You never see it arrive, move or leave. All of that happens out of shot, and it waits for the camera to settle if you're whipping it round. It wakes with the first note, or on its own if you haven't found one after a minute and a half. It turns up just out of view, somewhere with a clear line to you, with a soft sound from that side, so a turn of the head finds it. A low murmur tells you which side it's on. When you catch sight of it there's a sting, and the tape goes while you look: static, the lights failing, the sound rising. Look away and the tape recovers; look back and it's gone, or nearer. If nobody looks, it creeps closer, behind you, and when it's close the picture breaks up whichever way you face. Stand still and it gets you, so keep moving. Stare, or let it reach you, and the tape ends. It comes more often, and closer, with every note, and the music adds a layer as you go.
